...
def tekort_snel(df2, battery=300, zuinig=1.25):  # What does this function actually do?
    ...
    for i, row in df2.reset_index().iterrows():
        ...
        tekort_snel = max(
            0, df2.iloc[0:i+1].Afstand.sum()*zuinig - sum(tekort1) - sum(tekort2))
        tekort2.append(tekort_snel)
    return_df = pd.DataFrame(
        {'bijladen_snel': tekort2, 'bijladen': tekort1}, index=df2.index)
    return return_df
...
def simulate2(df2, zuinig=1.25, laadvermogen=44, laadvermogen_snel=150, aansluittijd=600, battery=300, nachtladen=0, activiteitenladen=0):
    ...
    if (nachtladen == 0) & (activiteitenladen == 0):
        df2['Laadtijd'] = np.where(
            (df2['thuis'] == 1), df2['Duur'], 0)  # alleen thuis laden
    elif (nachtladen == 1) & (activiteitenladen == 0):
        df2['Laadtijd'] = np.where((df2['thuis'] == 1) | (
            df2['nacht'] == 1), df2['Duur'], 0)  # thuis of 's nachts laden
    elif (nachtladen == 0) & (activiteitenladen == 1):
        df2['Laadtijd'] = np.where((df2['thuis'] == 1) | (
            df2['Laden'] == 1), df2['Duur'], 0)  # thuis of tijdens activiteit
    elif (nachtladen == 1) & (activiteitenladen == 1):
        df2['Laadtijd'] = np.where((df2['thuis'] == 1) | (df2['Laden'] == 1) | (
            df2['nacht'] == 1), df2['Duur'], 0)  # thuis, 's nachts of tijdens activiteit

    df2['Laadtijd'] = np.where((df2['Activiteit'] == 'Rijden') | (
        df2['Afstand'] >= 3), 0, df2['Laadtijd'])  # niet AC-laden tijdens rijden
    df2['laad_potentiaal1'] = df2['Laadtijd'].apply(lambda x: min(battery,
                                                                  max(0, ((x-aansluittijd)/3600))*laadvermogen))
    df2 = df2.merge(tekort_snel(df2, battery=battery, zuinig=zuinig),
...
def simulate(df2, zuinig=1.25, laadvermogen=44, laadvermogen_snel=150, aansluittijd=600, battery=300, nachtladen=0, activiteitenladen=0):
    ...
    for i in range(df2.shape[0]):
        ...
        bijladen_update = min(
            laadvermogen*(max(0, df2.iloc[i]['Laadtijd']-aansluittijd)/3600), battery - energie_update)
        energie_update = energie_update + bijladen_update
        bijladen.append(bijladen_update)
        energy.append(energie_update)
    return_df = pd.DataFrame({'energie': energy[:-1],
...
def berekenen_bijladen_spread(bijladen, laadvermogen, n_hours):
    a = ([laadvermogen]*int(bijladen/laadvermogen)) + [bijladen % laadvermogen]
    a += [0] * (n_hours - len(a))
    return a


def berekenen_bijladen_spread_smart(bijladen, n_hours):
...
def charge_hour(df, laadvermogen=44, laadvermogen_snel=150, aansluittijd=600, battery=300, smart=0):
    ...
    df_hour = df_bijladen.explode(column='StartTime').reset_index()[
        ['index', 'StartTime', 'bijladen', 'thuis']]

    if smart == 0:
        df_hour['bijladen'] = df_hour.groupby('index').bijladen.transform(
            lambda row: berekenen_bijladen_spread(row.max(), laadvermogen, len(row)))
    elif smart == 1:
        df_hour['bijladen'] = df_hour.groupby('index').bijladen.transform(
            lambda row: berekenen_bijladen_spread_smart(row.max(), laadvermogen, len(row)))
...
def process_excel_file(file, battery, zuinig, aansluittijd, laadvermogen, laadvermogen_snel, nachtladen, activiteitenladen, snelwegladen):
    ...
    optional_columns = df.columns.difference(
        ['Voertuig', 'Begindatum en -tijd', 'Einddatum en -tijd', 'Positie', 'Afstand', 'Activiteit'])

    df = df.sort_values(['Voertuig', 'Begindatum en -tijd']
    ...
    rows = rows.rename(columns={
                       'Begindatum en -tijd': 'Einddatum en -tijd', 'lag': 'Begindatum en -tijd'})
    ...
    if df.Voertuig.nunique() == 1:
        df['activiteit_g'] = (df['Activiteit'] != df.shift().fillna(
            method='bfill')['Activiteit']).cumsum()
    else:
    ...
    df = df.groupby(['Voertuig', 'activiteit_g']).agg(
        agg_dict).reset_index(drop=False).drop('activiteit_g', axis=1)

    df['Duur'] = (df['Einddatum en -tijd'] -
                  df['Begindatum en -tijd']).apply(lambda x: x.total_seconds())
    df['nacht'] = np.where(((df.Afstand < 3) & (df.Duur > 6*3600)), 1, 0)

    if df.Voertuig.nunique() == 1:
        df['RitID'] = (df['nacht'] < df.shift().fillna(
            method='bfill')['nacht']).cumsum()
    else:
    ...
    df['vertraging'] = np.where(
        df['bijladen_snel'] > 0, 600 + (3600*df['bijladen_snel']/laadvermogen_snel), 0)

    return df.drop(['index'], axis=1)


def show_haalbaarheid(df):
    df['Datum'] = df.groupby(['Voertuig', 'RitID'])[
        'Begindatum en -tijd'].transform(lambda x: x.min().date())

    haalbaarheid = df.pivot_table(values='energie', index='Voertuig', columns=[
                                  'Datum'], aggfunc=lambda x: 1 if min(x) >= -0.01 else 0)
    cmap = LinearSegmentedColormap.from_list('rg', ["r", "y", "g"], N=256)
    st.subheader('Haalbaarheid van de planning per dag en voertuig')
    fig1 = plt.figure(figsize=(10, 4))
    sns.heatmap(haalbaarheid, annot=False, cmap=cmap, vmin=0,
                vmax=1, linewidths=1, linecolor='black', cbar=False)
...
def show_demand_table(df):
    ...
    st.table(bijladen.reset_index(
        drop=True).loc[lambda d: d['Hoeveelheid energie geladen (kWu)'] > 0].head(10))
...
def plot_demand(df, battery, zuinig, aansluittijd, laadvermogen, laadvermogen_snel):
    ...
    charge_locations = df.loc[lambda d: d.bijladen > 0].groupby(
        'Positie').bijladen.sum().sort_values(ascending=False).index
    filter_option = st.selectbox(
        'Selecteer een locatie', charge_locations, index=0)
    ...
    n_days = max(1, (df['Begindatum en -tijd'].max() -
                 df['Begindatum en -tijd'].min()).days+1)

    plot_data1 = (charge_hour(df_plot, smart=0, battery=battery, aansluittijd=aansluittijd,
                  laadvermogen=laadvermogen).groupby('hour').bijladen.sum()/n_days)
    plot_data1 = df_hour_24h.merge(
        plot_data1, how='left', left_on='hour', right_index=True).fillna(0).set_index('hour')
    plot_data1.rename(
        columns={'bijladen': 'zonder smart charging'}).plot(ax=ax1)

    plot_data2 = (charge_hour(df_plot, smart=1, battery=battery, aansluittijd=aansluittijd,
                  laadvermogen=laadvermogen).groupby('hour').bijladen.sum()/n_days)
    plot_data2 = df_hour_24h.merge(
        plot_data2, how='left', left_on='hour', right_index=True).fillna(0).set_index('hour')
    plot_data2.rename(columns={'bijladen': 'met smart charging'}).plot(
        ax=ax1, color='red')
    ...
    # Save the DataFrame to BytesIO as an Excel file
        with pd.ExcelWriter(excel_data, engine='xlsxwriter') as writer:
            for location in charge_locations:
                temp = (charge_hour(df.loc[df.Positie == location], smart=0, battery=battery,
                        aansluittijd=aansluittijd, laadvermogen=laadvermogen).groupby('hour').bijladen.sum()/n_days)
                temp = df_hour_24h.merge(
                    temp, how='left', left_on='hour', right_index=True).fillna(0).set_index('hour')
        ...
        with pd.ExcelWriter(excel_data2, engine='xlsxwriter') as writer:
            for location in charge_locations:
                temp = (charge_hour(df.loc[df.Positie == location], smart=1, battery=battery,
                        aansluittijd=aansluittijd, laadvermogen=laadvermogen).groupby('hour').bijladen.sum()/n_days)
                temp = df_hour_24h.merge(
                    temp, how='left', left_on='hour', right_index=True).fillna(0).set_index('hour')
                temp.to_excel(writer, index=True, sheet_name=location[:31])
        ...
        with col1:
            st.download_button('Download de data voor regulier laden',
                               excel_data, file_name='data_demand_plot.xlsx')
        with col2:
            st.download_button('Download de data voor smart charging',
                               excel_data2, file_name='data_demand_plot_smart.xlsx')


def download_excel(df: pd.DataFrame) -> None:
    """
    Downloads a DataFrame as an Excel file and provides a download button in a Streamlit app.

    Args: df (pd.DataFrame): The DataFrame to be downloaded.
    ...
    # Offer the file download
    st.download_button('Download Excelbestand met modeluitkomsten',
                       excel_data, file_name='laadmodel_resultaten.xlsx')


def download_template() -> None:
    """
    Downloads a template file from a URL and provides a download button in a Streamlit app.
    ...
    if response.status_code != 200:
        st.error("Failed to download the template file.")
        return
    ...
    # Offer the file download
    st.download_button('Download Template', template_data,
